...
export default function CPDashboard() {
  ...
  useEffect(() => {
    ...
    fetchDashboard(period).then(res => { if (res) setData(res); setLoading(false); });
  ...
  const followUpsToday = myTasks.filter(t => t.dueDate?.startsWith(todayStr) && t.status !== 'completed').length;
  const callbacksToday = myLeads.filter(l => l.disposition === 'Callback Requested').length;
  ...
  const activityData = [
    ...
    { label: 'Follow-ups Done', value: myTasks.filter(t => t.status === 'completed').length, target: 15, icon: CheckCircle, color: '#10b981' },
  ];
    ...
    <div className="animate-fadeIn">
            ...
            {activityData.map((act, i) => (
              <div key={i} style={{ padding: 16, background: 'var(--bg-surface)', borderRadius: 16, border: '1px solid var(--border-subtle)' }}>
                ...
                <div style={{ height: 4, background: 'var(--border-subtle)', borderRadius: 2 }}>
                  <div style={{ height: '100%', width: `${Math.min(100, (act.value / act.target) * 100)}%`, background: act.color, borderRadius: 2 }}></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="studio-card" style={{ padding: 24, background: 'linear-gradient(135deg, #1e1b4b 0%, #312e81 100%)', color: 'white' }}>
          ...
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ height: 12, background: 'rgba(255,255,255,0.1)', borderRadius: 6, overflow: 'hidden' }}>
              <div style={{ height: '100%', width: `${progressPercent}%`, background: 'linear-gradient(90deg, #10b981, #34d399)', borderRadius: 6 }} />
            </div>
          </div>
      ...
      <div className="form-grid" style={{ gridTemplateColumns: '1fr 1.2fr', gap: 24 }}>
        ...
        <div className="studio-card" style={{ padding: 24 }}>
          ...
          <button className="btn btn-ghost" style={{ width: '100%', marginTop: 16, fontSize: 13, fontWeight: 600 }} onClick={() => navigate('/cp-work-queue')}>
