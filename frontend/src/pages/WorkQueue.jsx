...
import { 
  ...
  ArrowRight, Filter, Search, ChevronDown, Check
} from 'lucide-react';

export default function WorkQueue() {
  const { currentUser, leads, tasks, updateTask, addInteraction } = useApp();
  ...
  const todayFollowUps = bdeTasks.filter(t => t.dueDate === today && t.status !== 'completed');
  const overdueFollowUps = bdeTasks.filter(t => t.dueDate < today && t.status !== 'completed');
  const upcomingFollowUps = bdeTasks.filter(t => t.dueDate > today && t.status !== 'completed');
  
  // Callbacks could be a specific disposition or tag
  const callbackQueue = bdeLeads.filter(l => l.disposition === 'Callback Requested');
  
  const priorityLeads = bdeLeads.filter(l => l.priority === 'High' && l.status === 'active');
  ...
  const handleAction = (leadId, action) => {
    // This would normally trigger a modal or API call
  ...
  const renderTaskCard = (item, type) => {
    ...
    return (
      <div key={item.id} className="studio-card animate-fadeIn" style={{ padding: '16px 20px', marginBottom: 12, borderLeft: `4px solid ${item.priority === 'High' ? '#ef4444' : '#6366f1'}` }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
              <span style={{ fontSize: 15, fontWeight: 700, color: 'var(--text-primary)' }}>{lead.companyName || item.title}</span>
              {item.priority === 'High' && <span className="badge badge-danger" style={{ fontSize: 10, padding: '2px 6px' }}>HIGH PRIORITY</span>}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 12, color: 'var(--text-secondary)' }}>
              ...
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><Clock size={12} /> Last: {lead.createdAt}</span>
              <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}><MessageSquare size={12} /> {lead.disposition}</span>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            <button className="btn btn-ghost btn-sm" style={{ padding: '6px', color: '#10b981' }} title="Mark Complete" onClick={() => updateTask(item.id, { status: 'completed' })}>
              <CheckCircle size={18} />
            </button>
        ...
        {item.notes && (
          <div style={{ marginTop: 12, padding: '8px 12px', background: 'var(--bg-surface)', borderRadius: 8, fontSize: 12, color: 'var(--text-secondary)', border: '1px dashed var(--border-subtle)' }}>
    ...
    <div className="animate-fadeIn">
      ...
      <div className="form-grid" style={{ gridTemplateColumns: '1.5fr 1fr', gap: 24, alignItems: 'start' }}>
        ...
        <div>
            ...
            <>
              {todayFollowUps.length > 0 && (
                <div style={{ marginBottom: 24 }}>
                  <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Today's Follow-ups</h3>
              ...
              {callbackQueue.length > 0 && (
                <div style={{ marginBottom: 24 }}>
                  <h3 style={{ fontSize: 13, fontWeight: 700, color: '#06b6d4', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Callbacks Queue</h3>
              ...
              {todayFollowUps.length === 0 && callbackQueue.length === 0 && (
                <div className="studio-card" style={{ padding: 40, textAlign: 'center', color: 'var(--text-muted)' }}>
                  <CheckCircle size={48} style={{ marginBottom: 16, opacity: 0.2 }} />
                  <p>All clear for today! No pending follow-ups or callbacks.</p>
                </div>
              )}
            </>
          )}
          ...
          {activeTab === 'overdue' && (
            <div style={{ marginBottom: 24 }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: '#ef4444', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Overdue Execution</h3>
              {overdueFollowUps.length > 0 ? overdueFollowUps.map(t => renderTaskCard(t, 'task')) : <p style={{ color: 'var(--text-muted)', fontSize: 14 }}>Great job! No overdue tasks.</p>}
            </div>
          )}

          {activeTab === 'upcoming' && (
            <div style={{ marginBottom: 24 }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: 'var(--text-muted)', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Upcoming Priorities</h3>
          ...
          {activeTab === 'priority' && (
            <div style={{ marginBottom: 24 }}>
              <h3 style={{ fontSize: 13, fontWeight: 700, color: '#8b5cf6', marginBottom: 16, textTransform: 'uppercase', letterSpacing: '0.05em' }}>High Value Leads</h3>
        ...
        <div style={{ position: 'sticky', top: 24 }}>
          <div className="studio-card" style={{ padding: 24, marginBottom: 24, background: 'linear-gradient(135deg, var(--bg-card) 0%, rgba(99, 102, 241, 0.05) 100%)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div style={{ background: 'var(--bg-surface)', padding: 16, borderRadius: 12, border: '1px solid var(--border-subtle)' }}>
                ...
                <div style={{ fontSize: 20, fontWeight: 700, color: '#10b981' }}>12</div>
              </div>
              <div style={{ background: 'var(--bg-surface)', padding: 16, borderRadius: 12, border: '1px solid var(--border-subtle)' }}>
                <div style={{ fontSize: 11, color: 'var(--text-muted)', marginBottom: 4 }}>Call Connection</div>
                <div style={{ fontSize: 20, fontWeight: 700, color: '#6366f1' }}>65%</div>
              </div>
            </div>
            ...
            <div style={{ marginTop: 24 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, marginBottom: 8 }}>
                ...
                <span style={{ fontWeight: 600 }}>12/20 Calls</span>
              </div>
              <div style={{ height: 8, background: 'var(--bg-surface)', borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: '60%', background: 'linear-gradient(90deg, #6366f1, #8b5cf6)', borderRadius: 4 }}></div>
              </div>
            </div>
          </div>
          ...
          <div className="studio-card" style={{ padding: 24 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
              ...
              <h3 style={{ fontSize: 15, fontWeight: 700 }}>AI Suggested Next</h3>
            </div>
            <div style={{ background: 'var(--bg-surface)', padding: 16, borderRadius: 12, border: '1px solid #f59e0b40' }}>
              <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 4 }}>TechNova Solutions</div>
              <p style={{ fontSize: 12, color: 'var(--text-secondary)', lineHeight: 1.5 }}>
                High intent score (85). Suresh viewed the proposal twice in the last hour. Best time to call: <b>Now</b>.
              </p>
